...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $c_id = $_POST['custID'];
    $c_pass = password_hash($_POST['cust_pass'], PASSWORD_DEFAULT);
    $c_gender = $_POST['cust_gender'];
    $c_age = $_POST['cust_age'];

    $stmt = $conn->prepare("INSERT INTO customer (cust_firstname, cust_lastname, cust_email, cust_phone, custID, cust_password, cust_gender, cust_age) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $c_fn, $c_ln, $c_email, $c_phone, $c_id, $c_pass, $c_gender, $c_age);

    if ($stmt->execute()) {
        header("Location: customerdetailsinformation.php");
        exit();
    } else {
